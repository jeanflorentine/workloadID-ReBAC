...
module "helm_spire" {
  source = "../../modules/helm_spire"
  count  = var.enable_spire ? 1 : 0

  depends_on = [module.kube_namespaces]

  namespace       = var.spire_namespace
  release_name    = var.spire_release_name
  chart_version   = var.spire_chart_version
  trust_domain    = var.spire_trust_domain
  timeout_seconds = var.spire_timeout_seconds
}

module "helm_openbao" {
  source = "../../modules/helm_openbao"
  ...
  timeout_seconds = var.openbao_timeout_seconds
}

module "helm_minio" {
  source = "../../modules/helm_minio"
  ...
  timeout_seconds = var.minio_timeout_seconds
}

module "helm_openfga" {
  source = "../../modules/helm_openfga"
  count  = var.enable_openfga ? 1 : 0

  depends_on = [module.kube_namespaces]

  namespace        = var.openfga_namespace
  release_name     = var.openfga_release_name
  chart_version    = var.openfga_chart_version
  datastore_engine = var.openfga_datastore_engine
  log_level        = var.openfga_log_level
  timeout_seconds  = var.openfga_timeout_seconds
}
